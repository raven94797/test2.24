...
function runSelfCheck() {
    ...
    console.group('🚀 应用自检 - 编辑系统');
    
    checks.push({
        name: '配置对象',
        status: !!CONFIG && typeof CONFIG === 'object',
        detail: CONFIG ? `配置存在: ${Object.keys(CONFIG).length} 个键` : '配置未定义'
    });
    
    console.group('📝 编辑系统DOM元素');
    const editElements = [
        'editToolbar', 'editContentBtn', 'editorContainer', 'formatToolbar',
        'contentEditor', 'saveContentBtn', 'cancelEditBtn', 'editStatus'
    ];
    ...
    console.group('📄 内容显示DOM元素');
    const contentElements = ['wikiContent', 'contentLoading'];
    ...
    console.group('⚙️ 编辑系统函数');
    const editFunctions = [
        'enableEditing', 'saveContent', 'cancelEditing', 
        'applyFormat', 'updateEditStatus', 'parseMarkdown', 'htmlToMarkdown'
    ];
    ...
    checks.push({
        name: 'JSONBin配置',
        status: !!CONFIG.JSONBIN_API_URL && !!CONFIG.JSONBIN_MASTER_KEY,
        detail: `API URL: ${CONFIG.JSONBIN_API_URL ? '已设置' : '未设置'}, Key: ${CONFIG.JSONBIN_MASTER_KEY ? '已设置' : '未设置'}`
    });
    
    console.group('📂 页面结构');
    const expectedStructure = {
        'Project': ['description', 'design', 'contribution', 'implementation', 'safety'],
        'Wet Lab': ['engineering', 'notebook', 'result', 'protocol', 'parts', 'proof-of-concept', 'future'],
        'Human Practice': ['ihp', 'education', 'inclusivity', 'collaboration']
    };
    
    Object.keys(expectedStructure).forEach(category => {
        console.group(`${category}:`);
        expectedStructure[category].forEach(page => {
            const hasId = !!CONFIG.BIN_IDS[page];
            checks.push({
                name: `页面: ${page}`,
                status: hasId,
                detail: hasId ? `✅ BIN_ID: ${CONFIG.BIN_IDS[page]}` : '❌ 缺失'
            });
            if (hasId) passed++; else failed++;
            console.log(`  ${hasId ? '✅' : '❌'} ${page}`);
        });
        console.groupEnd();
    });
    console.groupEnd();
    ...
    checks.push({
        ...
        detail: `找到 ${navLinks.length} 个导航链接`
    });
    if (navLinks.length > 0) passed++; else failed++;
    
    const editBtn = document.getElementById('editContentBtn');
    checks.push({
        name: '编辑按钮可见性',
        status: !!editBtn && editBtn.style.display !== 'none',
        detail: editBtn ? '✅ 可见' : '❌ 不可见'
    });
    if (editBtn && editBtn.style.display !== 'none') passed++; else failed++;
    
    const editToolbar = document.getElementById('editToolbar');
    checks.push({
        name: '编辑工具栏可见性',
        status: !!editToolbar && editToolbar.style.display !== 'none',
        detail: editToolbar ? '✅ 可见' : '❌ 不可见'
    });
    if (editToolbar && editToolbar.style.display !== 'none') passed++; else failed++;
    
    console.group('📊 自检结果汇总');
    ...
    console.log(`当前页面: ${CONFIG.currentPage}`);
    console.log(`调试模式: ${CONFIG.DEBUG_MODE ? '开启' : '关闭'}`);
    console.log(`纸张模式: ${CONFIG.paperMode ? '开启' : '关闭'}`);
    console.groupEnd();
    console.groupEnd();
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { initApp, initNavigation, loadWikiContent, renderWikiContent, enableEditing, saveContent, cancelEditing, applyFormat, bindEventListeners, updateEditStatus, showError, runSelfCheck };
} else {
